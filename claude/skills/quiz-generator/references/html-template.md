...
**答えが見えている問題集は問題集ではない。** 2026-08-08 に3種類とも実際にやった。

| 漏れる場所 | やってしまった例 | 正しい形 |
| --- | --- | --- |
| **出典の見出し** | `◆ p.18 ⑧　亜寒帯の植生　タイガ` | `◆ p.18 ⑧　寒暖の差が激しい土地に暮らす人々（教科書P.38・39）` |
| **同じ節の別の設問** | 関連問題の文が「タイガが針葉樹ばかりで…」 | 「シベリアに広がる森林が針葉樹ばかりで…」 |
| **設問文のヒント** | 「中心部ではなくどこに造られますか」 | 「どこに造られますか」 |

- **h2 の見出しには、その問題が出た単元名・節名だけを書く。**
  何を問うているかを見出しで説明しない（説明はそのまま答えの言い換えになる）。
  **ワークの節見出しをそのまま使うのが最も安全**
- **同じ節に複数の設問を置くときは、先の設問の答えを後の設問文に書かない。**
  「その語を前提に理由を問う」形は自然に見えるが、前の答えを明かしている
- 生成後に **`references/check_leak.py` を必ず通す**。見出しと設問文の漏れは機械で見つかる。
  設問文のヒント（3行目）は機械では拾えないので目で読む

### そのほかの大原則

- **出力はHTML1本**。docx・PDFは作らない。印刷が要るときはブラウザの「印刷 → PDFに保存」を使う
- **Markdown記法を本文に書かない**。`**強調**` はHTMLでは効かず `**` がそのまま画面に出る
  （2026-08-08 に解説文で10か所やった）。強調が要るなら文の組み立てで示す
