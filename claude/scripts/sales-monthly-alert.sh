...
#       ①未→「DBから取得して整理」／①済②未→「サマリーHTML作成が残り」。
#       ※2026-09-02 専務指示でドライブへの生データ配置は廃止＝取得は業務DB直結（fetch_db_sales.py）。
...
if [ -z "$XLSX" ]; then
  # 段階①未：Claudeがやること（DBから取得して整理）を耳打ち
  echo "  得意先別売上データが未整理です。"
  echo "  → 業務DBから取得して整理します（承認のうえ fetch_db_sales.py → build_master → build_report）。"
else
